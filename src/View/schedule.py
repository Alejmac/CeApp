...
class ScheduleView:
    ...
    def build(self, page: Page):
        ...
        # Definir el horario (esto es solo un ejemplo, ajusta según tu lógica)
        horario = {
            "Lunes": {"Materia": "Matemáticas", "Hora": "8:00 - 10:00"},
            "Martes": {"Materia": "Ciencias", "Hora": "10:00 - 12:00"},
            "Miércoles": {"Materia": "Historia", "Hora": "12:00 - 14:00"}
        }

        # Crear los ExpansionTiles para cada día
        expansion_tiles = [
            ExpansionTile(
                title=Text(f"{dia}", size=14, weight="bold", color=ft.colors.BLACK),
                ...
                on_change=lambda e: print(f"ExpansionTile {dia} {'expanded' if e.data=='true' else 'collapsed'}")
            ) for dia, detalles in horario.items()
        ]
